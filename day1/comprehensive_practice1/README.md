...
- `Tuple` import 누락 같은 단순 실수뿐 아니라, `except Exception`으로 감싸고 하드코딩된 fallback을 반환하는 패턴이 왜 나쁜지(예외를 조용히 무마하면 실제 장애 상황을 놓친다), `print()` 대신 `logging`을 왜 써야 하는지를 직접 코드를 고치면서 체감했다.
